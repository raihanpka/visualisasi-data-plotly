// Fungsi utama untuk memulai proses pembuatan plot
function makeplot() {
...
allRows.forEach(row => {
  ...
  // Menambahkan data luas panen ke sumbu x dan produksi ke sumbu y
...
const traces = Object.keys(groupedData).map(province => ({
  x: groupedData[province].x, // Data sumbu x (luas panen)
...
const xData = allRows.map(row => parseFloat(row['Luas Panen'])); // Mengambil semua data luas panen
const yData = allRows.map(row => parseFloat(row['Produksi'])); // Mengambil semua data produksi

const n = xData.length; // Jumlah data
const xMean = xData.reduce((sum, val) => sum + val, 0) / n; // Rata-rata luas panen
...
// Fungsi untuk membuat plot menggunakan Plotly
function makePlotly(traces) {
const plotDiv = document.getElementById("scatter"); // Elemen HTML tempat grafik akan ditampilkan
const layout = {
  title: { 
    text: 'Hubungan Luas Panen dengan Hasil Produksi Padi di Pulau Sumatera (Tahun 1993-2020)' 
  }, // Judul grafik
  xaxis: { title: 'Luas Panen (ha)' }, // Label sumbu x
